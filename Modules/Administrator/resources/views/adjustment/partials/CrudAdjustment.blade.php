<div class="modal fade" id="modalCrudAdjustment" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel2" aria-hidden="true">
    <div class="modal-xl modal-dialog modal-dialog-slideout" role="document">
        <div class="modal-content">
            ...
            <form method="post" class="" data-parsley-validate id="formCrudAdjustment">
                @csrf()
                <input type="text" hidden name="action" id="CrudAdjustmentAction" />
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-12 pr-1 pl-1">
                            <table id="gridAdjustmentList"></table>
                            <div id="pagerGridAdjustmentList"></div>
                        </div>
            ...
            <div id="CrudAdjustmentError">

            </div>
            <div id="CrudAdjustmentAlertDelete"></div>
        </div>
    </div>
</div>
...
<script>
    ...
    // List material Adjustment
    $("#gridAdjustmentList").jqGrid({
        datatype: "local",
        data: [],
        colModel: [{
                name: 'id',
    ...
    jQuery("#gridAdjustmentList").jqGrid('setGroupHeaders', {
        ...
        groupHeaders: [{
            ...
            numberOfColumns: 3,
            titleText: 'Adjustment'
        }]
    });

    $("#gridAdjustmentList").navGrid("#pagerGridAdjustmentList", {
        add: false,
        edit: false,
        del: false,
        search: false,
    ...
    function actionListMaterial(values, options, rowObject) {
        ...
        var btn = `<button ${lock} type="button" data-id="${btnid}" onclick="CrudListAdjustment('update','${btnid}')"  class="btn btn-sm text-white btn-option badge-success btnActionMaterial"><i class="fa fa-pencil"></i></button>`;
        btn += `<button ${lock} type="button" data-id="${btnid}" onclick="CrudListAdjustment('delete','${btnid}')" class="btn btn-sm text-white btn-option badge-danger btnActionMaterial"><i class="fa fa-remove"></i></button>`;
        return btn;
    }


    function reloadgridAdjustmentList(data) {
        // Clear existing data
        $("#gridAdjustmentList").jqGrid('clearGridData', true);
        $("#gridAdjustmentList").jqGrid('setGridParam', {
            data: data
        });
        // Refresh the grid
        $("#gridAdjustmentList").trigger('reloadGrid');
    }


    // Adjust grid size when modal is shown
    $('#modalCrudAdjustment').on('shown.bs.modal', function() {
        $("#gridAdjustmentList").jqGrid('setGridWidth', $(".modal-dialog").width() * 0.98); // Adjust the width as needed
    });


    function CrudListAdjustment(action, idx) {
        var Grid = $('#gridAdjustmentList'),
            selRowId = idx,
        ...
        $("#QtyUnitAdjustment").val(qtyUnit)
        $("#QtyUnitsAdjustment").val(qtyUnits)
        $("#QtyPackagingAdjustment").val(qtyPackaging)
    ...
    function AddNewMaterialModal() {
        ...
        $("#formCrudAdjustment .form-control").each(function() {
            var $this = $(this);
            if ($this.val().trim() === "") {
                errorCount++;
            }
    ...
    $('#formCrudAdjustment').submit(function(e) {
        e.preventDefault();
        var f = $(this);
        f.parsley().validate();

        if (f.parsley().isValid()) {
            var formData = new FormData($('#formCrudAdjustment')[0]);
            var actions = $("#CrudAdjustmentAction").val();
            var url = '';
            if (actions == "create") {
                url = '{{ url("administrator/jsonCreateAdjustment") }}';
            } else if (actions == "update") {
                url = '{{ url("administrator/jsonUpdateAdjustment") }}';
            } else if (actions == "delete") {
                url = '{{ url("administrator/jsonDeleteAdjustment") }}';
            } else if (actions == "putaway") {
                url = '{{ url("administrator/jsonPutawayAdjustment") }}';
            }
            ...
            var data = {
                ...
                'type_adjust': $("#type_adjust").val(),
                'remarks': $("textarea[name=remarks]").val(),
                'dataMaterial': JSON.stringify(dataMaterialAdjustment)
            }
            console.log(data);
            $.ajax({
                url: url,
