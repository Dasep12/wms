...
--
-- Host: localhost    Database: bitwms
-- ------------------------------------------------------
...
INSERT INTO `tbl_trn_detailshipingmaterial` VALUES (22,14,50,'HK284-0W218-00','PAPER 218','FES2','ROLLS','METER','SKID',12,120,3,0,0,0,'','2024-08-15 13:57:25','8','2024-08-15 13:58:23','8'),(23,15,50,'HK284-0W218-00','PAPER 218','FES2','ROLLS','METER','SKID',8,80,2,12,120,3,NULL,'2024-08-15 13:59:35','8','2024-08-15 13:59:38','8');
...
INSERT INTO `tbl_trn_shipingmaterial` VALUES (14,1,'0001/ADJUST/RIM/TBINA/VIII/2024',NULL,NULL,NULL,NULL,NULL,'close','2024-08-15 13:58:23','2024-08-15 13:58:23','in','Adjust','2024-08-15 13:57:25','new items',NULL,'2024-08-15 13:57:25','2024-08-15 13:58:23','8'),(15,1,'001/SJ/TBINA/VIII/2024','REF-022',NULL,'JL RAYA BEKASI','Dasep','BU 23 UI','close','2024-08-15 13:59:38',NULL,'in','Order','2024-08-15 13:59:35',NULL,'8','2024-08-15 13:59:35','2024-08-15 13:59:38','8');
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `vw_tbl_control_stock` AS select `a`.`id` AS `id`,`a`.`customers_id` AS `customers_id`,`a`.`uniqueId` AS `uniqueId`,`b`.`name_unit` AS `units`,`c`.`name_unit` AS `unit`,`d`.`name_packaging` AS `packaging`,`a`.`no_material` AS `no_material`,`a`.`name_material` AS `name_material`,ifnull(`x`.`QtyUnit`,0) - ifnull(`y`.`QtyUnit`,0) AS `QtyUnit`,ifnull(`x`.`QtyUnits`,0) - ifnull(`y`.`QtyUnits`,0) AS `QtyUnits`,ifnull(`x`.`QtyPackaging`,0) - ifnull(`y`.`QtyPackaging`,0) AS `QtyPackaging`,coalesce(`y`.`updated_at`,`x`.`updated_at`) AS `updated_at` from (((((`bitwms`.`tbl_mst_material` `a` left join `bitwms`.`tbl_mst_units` `b` on(`b`.`id` = `a`.`unit_id`)) left join `bitwms`.`tbl_mst_units` `c` on(`c`.`id` = `a`.`parentUnitId`)) left join `bitwms`.`tbl_mst_packaging` `d` on(`d`.`id` = `a`.`packaging_id`)) left join (select `ttd`.`material_id` AS `material_id`,sum(`ttd`.`qtyUnits`) AS `QtyUnits`,sum(`ttd`.`qtyUnit`) AS `QtyUnit`,sum(`ttd`.`qtyPackaging`) AS `QtyPackaging`,max(`ttd`.`updated_at`) AS `updated_at` from (`bitwms`.`tbl_trn_detailshipingmaterial` `ttd` left join `bitwms`.`tbl_trn_shipingmaterial` `tts` on(`tts`.`id` = `ttd`.`headers_id`)) where `tts`.`status` = 'close' and `tts`.`types` = 'in' group by `ttd`.`material_id` order by `ttd`.`updated_at` desc) `x` on(`x`.`material_id` = `a`.`id`)) left join (select `ttd`.`material_id` AS `material_id`,ifnull(sum(`ttd`.`qtyUnits`),0) AS `QtyUnits`,sum(`ttd`.`qtyUnit`) AS `QtyUnit`,sum(`ttd`.`qtyPackaging`) AS `QtyPackaging`,max(`ttd`.`updated_at`) AS `updated_at` from (`bitwms`.`tbl_trn_detailshipingmaterial` `ttd` left join `bitwms`.`tbl_trn_shipingmaterial` `tts` on(`tts`.`id` = `ttd`.`headers_id`)) where `tts`.`status` = 'close' and `tts`.`types` = 'out' group by `ttd`.`material_id`) `y` on(`y`.`material_id` = `a`.`id`)) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2024-08-16 14:04:25
